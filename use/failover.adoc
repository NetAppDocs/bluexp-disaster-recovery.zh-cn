...
[role="lead"]
发生灾难时、使用FSx for NetApp ONTAP将主内部站点故障转移到AWS站点上的VMware Cloud。
...
此过程不会占用Amazon FSx for NetApp ONTAP的额外物理容量。初始源卷不会被修改、即使在灾难恢复期间、副本作业也可以继续进行。
...
如果发生灾难、请使用FSx for NetApp ONTAP将源站点按需故障转移到AWS站点上的VMware Cloud。
...
* 数据存储库会根据灾难恢复计划中捕获的信息创建并挂载到VMC集群或主机。
* 目标虚拟机将根据在"资源组"页面中捕获的顺序进行注册并启动。
* SnapMirror关系会从目标虚拟机反转到源虚拟机。



TIP: 故障转移开始后、可以在VMC vCenter (虚拟机、网络和数据存储库)中看到恢复的项目。默认情况下、虚拟机会恢复到工作负载文件夹。
...
. 在Test failover页面中、输入灾难恢复计划名称进行确认、然后选择*故障转移*。
