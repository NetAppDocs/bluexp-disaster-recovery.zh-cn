...
有关在站点中包含子网映射的详细信息，请参见link:../use/sites-add.html["添加vCenter Server站点"]。
...
有关跳过保护的详细信息，请参见link:../use/failover.html["将应用程序故障转移到远程站点"]。
...
有关在复制计划中使用外部快照解决方案的详细信息，请参见link:../use/drplan-create.html["创建复制计划"]。
