...
* *支持使用FC*从内部到内部的VMware VMFS数据存储库：此版本包含一个_technology preview_支持在VMware vSphere虚拟机文件系统(VMFS)数据存储库上挂载的VM、以便将FC保护到内部存储。以前、该服务提供了一个技术预览版、支持iCI的VMFS数据存储库。在此版本中、BlueXP灾难恢复支持对内部VMware工作负载进行技术预览、并将技术预览迁移到具有FC VMFS数据存储库的内部VMware环境。
+
